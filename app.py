...
Run locally: pip install streamlit pandas numpy openpyxl yfinance plotly streamlit run app.py

Deploy (recommended for tablets): Streamlit Community Cloud using app.py + requirements.txt
...
Backtest: ATR stop/TP, fees+slippage, gap-aware stops, extra stop slippage, simple risk-based sizing

Walk-forward: time-based train/test split + out-of-sample metrics

Parameter sweep: grid search (stop_atr x take_atr) + stability view (heatmap)
...
yfinance is not execution-grade data. """
...
try: import plotly.graph_objects as go except Exception:  # pragma: no cover go = None
...
def rsi(close: pd.Series, period: int = 14) -> pd.Series: delta = close.diff() gain = delta.clip(lower=0) loss = -delta.clip(upper=0) avg_gain = gain.ewm(alpha=1 / period, adjust=False).mean() avg_loss = loss.ewm(alpha=1 / period, adjust=False).mean() rs = avg_gain / avg_loss.replace(0, np.nan) return 100 - (100 / (1 + rs))

def macd(close: pd.Series, fast: int = 12, slow: int = 26, signal: int = 9): macd_line = ema(close, fast) - ema(close, slow) signal_line = ema(macd_line, signal) hist = macd_line - signal_line return macd_line, signal_line, hist

def bollinger(close: pd.Series, period: int = 20, num_std: float = 2.0): mid = close.rolling(period).mean() sd = close.rolling(period).std(ddof=0) upper = mid + num_std * sd lower = mid - num_std * sd return mid, upper, lower

def atr(df: pd.DataFrame, period: int = 14) -> pd.Series: high, low, close = df["high"], df["low"], df["close"] prev_close = close.shift(1) tr = pd.concat( [(high - low), (high - prev_close).abs(), (low - prev_close).abs()], axis=1 ).max(axis=1) return tr.ewm(alpha=1 / period, adjust=False).mean()

def adx(df: pd.DataFrame, period: int = 14) -> pd.Series: """Average Directional Index (ADX) to estimate trend strength.""" high, low, close = df["high"], df["low"], df["close"]
...
plus_di = 100 * (pd.Series(plus_dm, index=df.index).ewm(alpha=1 / period, adjust=False).mean() / tr_sm)
minus_di = 100 * (pd.Series(minus_dm, index=df.index).ewm(alpha=1 / period, adjust=False).mean() / tr_sm)
...
def normalize_cols(df: pd.DataFrame) -> pd.DataFrame: mapping = {} for c in df.columns: lc = str(c).strip().lower() if lc in ["date", "datetime", "time", "timestamp"]: mapping[c] = "date" elif lc in ["open", "o"]: mapping[c] = "open" elif lc in ["high", "h"]: mapping[c] = "high" elif lc in ["low", "l"]: mapping[c] = "low" elif lc in ["close", "c", "adj close", "adj_close", "adjclose"]: mapping[c] = "close" elif lc in ["volume", "vol", "v"]: mapping[c] = "volume" return df.rename(columns=mapping)

def ensure_ohlcv(df: pd.DataFrame) -> pd.DataFrame: df = df.copy() if "high" not in df.columns: df["high"] = df["close"] if "low" not in df.columns: df["low"] = df["close"] if "open" not in df.columns: df["open"] = df["close"] if "volume" not in df.columns: df["volume"] = np.nan return df

def load_any(file) -> pd.DataFrame: name = file.name.lower() if name.endswith(".csv"): df = pd.read_csv(file) elif name.endswith(".xlsx") or name.endswith(".xls"): df = pd.read_excel(file) else: raise ValueError("Upload a .csv or .xlsx/.xls")
...
@st.cache_data(show_spinner=False) def load_live_cached(ticker: str, period: str, interval: str, auto_adjust: bool) -> pd.DataFrame: if yf is None: raise ValueError("Live mode needs yfinance. Install: pip install yfinance")
...
if data is None or data.empty:
    raise ValueError("No data returned. Try a different interval/period combo (e.g., 1y + 1d).")
...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame: df = df.copy() close = df["close"]
...
def add_signal(df: pd.DataFrame, use_adx_filter: bool, adx_threshold: float) -> pd.DataFrame: """Per-bar action + target position. 1=long, -1=short, 0=flat.""" df = df.copy()
...
df["rsi_state"] = np.where(rsi_overbought, "Overbought", np.where(rsi_oversold, "Oversold", "Neutral"))
df["trend_state"] = np.where(uptrend, "Uptrend", np.where(downtrend, "Downtrend", "Sideways / Transition"))
...
def compute_drawdown(equity: pd.Series) -> pd.Series: peak = equity.cummax() return (equity / peak) - 1.0

def annualization_factor(index: pd.Index) -> float: if not isinstance(index, pd.DatetimeIndex) or len(index) < 3: return 252.0 deltas = index.to_series().diff().dropna().dt.total_seconds().values if len(deltas) == 0: return 252.0 median_sec = float(np.median(deltas)) if median_sec <= 0: return 252.0 days = median_sec / 86400.0 return 365.0 / days

def backtest_atr_strategy( df: pd.DataFrame, initial_cash: float, risk_per_trade: float, stop_atr: float, take_atr: float, fee_bps: float, slippage_bps: float, stop_extra_slip_bps: float, allow_short: bool, fill_on_gap: bool, ) -> tuple[pd.DataFrame, pd.DataFrame, dict]: """Single-position backtest.
...
if not trades_df.empty:
    ...
    expectancy = float(trades_df["pnl"]
                       .mean())

    # Max consecutive losses
    loss_flag = (trades_df["pnl"] <= 0).astype(int).values
    streak = 0
    max_streak = 0
    for v in loss_flag:
        if v == 1:
    ...
    if "entry_time" in trades_df.columns and "exit_time" in trades_df.columns:
        # best-effort: count bars between entry & exit
        pos_map = {t: i for i, t in enumerate(idx)}
        for _, row in trades_df.iterrows():
            et = row.get("entry_time")
            xt = row.get("exit_time")
            if et in pos_map and xt in pos_map:
                hold_bars.append(max(0, pos_map[xt] - pos_map[et]))
    avg_hold = float(np.mean(hold_bars)) if hold_bars else np.nan
...
# Exposure: compute from actual backtest equity points (approx) by forward-filling position state
# For simplicity we approximate using signal intent; good enough for dashboard.
...
metrics = {
    ...
    "avg_holding_bars": float(avg_hold) if np.isfinite(avg_hold) else np.nan,
    "exposure_pct": exposure,
}

# Attach equity/drawdown to df for charts
...
def split_train_test(df: pd.DataFrame, test_fraction: float) -> tuple[pd.DataFrame, pd.DataFrame]: df = df.dropna().copy() n = len(df) if n < 50: return df, df.iloc[0:0] cut = int(np.floor(n * (1 - test_fraction))) cut = max(30, min(cut, n - 10)) return df.iloc[:cut], df.iloc[cut:]

def run_backtest_bundle(df: pd.DataFrame, params: dict) -> tuple[pd.DataFrame, pd.DataFrame, dict]: return backtest_atr_strategy( df, initial_cash=float(params["initial_cash"]), risk_per_trade=float(params["risk_per_trade"]), stop_atr=float(params["stop_atr"]), take_atr=float(params["take_atr"]), fee_bps=float(params["fee_bps"]), slippage_bps=float(params["slippage_bps"]), stop_extra_slip_bps=float(params["stop_extra_slip_bps"]), allow_short=bool(params["allow_short"]), fill_on_gap=bool(params["fill_on_gap"]), )

def sweep_params(df: pd.DataFrame, params: dict, stop_grid: list[float], take_grid: list[float]) -> pd.DataFrame: rows = [] for s in stop_grid: for t in take_grid: p = dict(params) p["stop_atr"] = float(s) p["take_atr"] = float(t) _, _, m = run_backtest_bundle(df, p) if "error" in m: continue rows.append( { "stop_atr": float(s), "take_atr": float(t), "total_return": float(m["total_return"]), "max_drawdown": float(m["max_drawdown"]), "profit_factor": float(m["profit_factor"]) if np.isfinite(m.get("profit_factor", np.nan)) else np.nan, "win_rate": float(m["win_rate"]) if np.isfinite(m.get("win_rate", np.nan)) else np.nan, "num_trades": int(m["num_trades"]), } ) return pd.DataFrame(rows)
...
def plot_candles(df: pd.DataFrame, overlays: list[str] | None = None, title: str = ""): overlays = overlays or []
...
def plot_heatmap(df_sweep: pd.DataFrame, value_col: str, title: str): if go is None: st.dataframe(df_sweep) return
...
with st.sidebar: st.header("Signal") use_adx_filter = st.checkbox("Use ADX filter (avoid chop)", value=True) adx_threshold = st.slider("ADX threshold", 5.0, 35.0, 18.0, 0.5)
...
def run_all(df0: pd.DataFrame, title: str): if len(df0) < 80: st.warning("Short dataset (<80 rows). Stats may be unreliable.")
...
# Tabs for deep-dive
t_over, t_bt, t_wf, t_sweep, t_trades = st.tabs(["Backtest", "Equity", "Walk-forward", "Sweep", "Trades"]) 
        ...
        st.write(
            {
                ...
                "Avg holding (bars)": metrics["avg_holding_bars"],
...
with t_sweep:
    ...
    st.caption("Grid search on stop_atr × take_atr using the full dataset. Use walk-forward for real validation.")
...
with mode_upload: st.subheader("Upload") uploaded = st.file_uploader("Upload CSV or Excel", type=["csv", "xlsx", "xls"], key="upload_file") st.caption("Best: Date/Open/High/Low/Close/Volume. Minimum: Close.")
...
with mode_live: st.subheader("Live") if yf is None: st.warning("Live mode needs yfinance: pip install yfinance")
